...
<<<<<<< HEAD
    ...
    public List<Version> listVersion(@PathVariable String ip){
        ...
        for (int i = 0; i < commandVersion.size(); i++) {
            ...
            versionList.add(new Version(command.getVersion()));
        }
        return versionList;
    }
    ...
    @ApiOperation(value = "查看指定网关微服务")
    @CrossOrigin
    @GetMapping("/service/{name}")
    public GwServState inquireServiceOne(@PathVariable String name) {
        Gateway gateway = gatewayService.findGatewayByName(name);
        String url =  "http://"+ gateway.getIp() +":8090/api/instance/service";
        GwServState gwServState = new GwServState(gateway.getIp(),gateway.getName(),false,new LinkedList<>());
        try{
            List<FileDescriptor> fileDescriptors = restTemplate.getForObject(url,List.class);
            gwServState.setState(true);
            gwServState.setFileDescriptorLists(fileDescriptors);
        }catch (Exception e){
            logService.error("retrieve","无法连接至网关"+gateway.getName()+":"+gateway.getIp()+" 异常:"+e.toString());
            gwServState.setState(false);
    ...
    @ApiOperation(value = "查看所有网关微服务")
    @CrossOrigin
    @GetMapping("/service")
    public List<GwServState> inquireService(){
        List<GwServState> gwServStates = new LinkedList<>();
        List<Gateway> allGateway = gatewayService.findAllGateway();
        for (Gateway gateway : allGateway) {
            String ip = gateway.getIp();
            String url =  "http://"+ ip +":8090/api/instance/service";
            GwServState gwServState = new GwServState(ip,gateway.getName(),false,new LinkedList<>());
            try{
                List<FileDescriptor> fileDescriptors = restTemplate.getForObject(url,List.class);
                gwServState.setState(true);
                gwServState.setFileDescriptorLists(fileDescriptors);
            ...
            gwServStates.add(gwServState);
        }
        return gwServStates;
    }

    @ApiOperation(value = "启动指定网关指定微服务")
    @CrossOrigin
    @PutMapping("/service/{name}")
    public boolean startService(@PathVariable String name,@RequestParam String jarName){
        Gateway gateway = gatewayService.findGatewayByName(name);
        String url =  "http://"+ gateway.getIp() +":8090/api/instance/service?jarName="+jarName;
...
<<<<<<< HEAD
    ...
    public boolean killService(@PathVariable String name,@RequestParam int port){
        Gateway gateway = gatewayService.findGatewayByName(name);
        String url =  "http://"+ gateway.getIp() +":8090/api/instance/service?port="+port;
        boolean success = false;
        try{
            restTemplate.delete(url,port);
            success = true;
        }catch (Exception e){
            logService.error("retrieve","无法连接至网关"+gateway.getName()+":"+gateway.getIp()+" 异常:"+e.toString());
            success = false;
